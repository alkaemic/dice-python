...
def test_drop_highest_basic():
    ...
    drop_highest(results, ModifierSpec(key="dh", argument=1), _rng(), _ctx(6))
...
def test_drop_highest_default_1():
    results = _results(3, 5, 1, 6)
    drop_highest(results, ModifierSpec(key="dh"), _rng(), _ctx(6))
    kept = [r for r in results if r.kept]
    assert len(kept) == 3
...
def test_drop_lowest_basic():
    results = _results(3, 5, 1, 6)
    drop_lowest(results, ModifierSpec(key="dl", argument=1), _rng(), _ctx(6))
    kept = [r for r in results if r.kept]
    assert len(kept) == 3
...
def test_drop_lowest_default_1():
    results = _results(3, 5, 1, 6)
    drop_lowest(results, ModifierSpec(key="dl"), _rng(), _ctx(6))
    kept = [r for r in results if r.kept]
    assert len(kept) == 3
...
def test_drop_lowest_multiple():
    ...
    drop_lowest(results, ModifierSpec(key="dl", argument=2), _rng(), _ctx(6))
    kept = [r for r in results if r.kept]
    assert len(kept) == 3
...
def test_drop_highest_zero_drops_nothing():
    results = _results(3, 5, 1, 6)
    drop_highest(results, ModifierSpec(key="dh", argument=0), _rng(), _ctx(6))
    assert all(r.kept for r in results)


def test_drop_lowest_zero_drops_nothing():
    results = _results(3, 5, 1, 6)
    drop_lowest(results, ModifierSpec(key="dl", argument=0), _rng(), _ctx(6))
    assert all(r.kept for r in results)


def test_drop_all_dice():
    results = _results(3, 5)
    drop_highest(results, ModifierSpec(key="dh", argument=2), _rng(), _ctx(6))
