...
def test_compound_no_match():
    ...
    rng = SeededRNG(0)
    out = compound(results, ModifierSpec(key="!!"), rng, ctx=_ctx(6))
    assert len(out) == 2
...
def test_compound_single_explosion():
    ...
    out = compound(results, ModifierSpec(key="!!"), rng, ctx=_ctx(2))
...
def test_compound_with_compare_point():
    ...
    rng = SeededRNG(42)
    out = compound(
        results, ModifierSpec(key="!!", compare_point=">=5"), rng, ctx=_ctx(6)
    )
...
def test_compound_max_explosions_exceeded():
    ...
    with pytest.raises(DiceExecutionError, match="MAX_EXPLOSIONS_EXCEEDED"):
        compound(results, ModifierSpec(key="!!"), rng, ctx=_ctx(1))
...
def test_compound_empty_list():
    ...
    rng = SeededRNG(0)
    out = compound([], ModifierSpec(key="!!"), rng, ctx=_ctx(6))
    assert out == []
