...
def test_clamp_min_raises_low_values():
    ...
    rng = SeededRNG(0)
    out = clamp_min(results, ModifierSpec(key="min", argument=2), rng, faces=6)
...
def test_clamp_min_equal_to_threshold():
    ...
    rng = SeededRNG(0)
    out = clamp_min(results, ModifierSpec(key="min", argument=3), rng, faces=6)
...
def test_clamp_max_lowers_high_values():
    ...
    rng = SeededRNG(0)
    out = clamp_max(results, ModifierSpec(key="max", argument=4), rng, faces=6)
...
def test_clamp_max_no_effect_when_all_below():
    ...
    rng = SeededRNG(0)
    out = clamp_max(results, ModifierSpec(key="max", argument=5), rng, faces=6)
...
def test_clamp_empty_list():
    ...
    rng = SeededRNG(0)
    out = clamp_min([], ModifierSpec(key="min", argument=2), rng, faces=6)
    assert out == []
