...
def test_target_greater_than():
    ...
    rng = SeededRNG(0)
    out = target(results, ModifierSpec(key=">", compare_point=">7"), rng, faces=10)
...
def test_target_less_than():
    ...
    rng = SeededRNG(0)
    out = target(results, ModifierSpec(key="<", compare_point="<3"), rng, faces=6)
...
def test_target_equal():
    ...
    rng = SeededRNG(0)
    out = target(results, ModifierSpec(key="=", compare_point="=5"), rng, faces=6)
...
def test_target_no_matches():
    ...
    rng = SeededRNG(0)
    out = target(results, ModifierSpec(key=">", compare_point=">10"), rng, faces=10)
...
def test_target_empty_list():
    ...
    rng = SeededRNG(0)
    out = target([], ModifierSpec(key=">", compare_point=">7"), rng, faces=10)
    assert out == []
