...
def test_cs_default_marks_max_value():
    ...
    rng = SeededRNG(0)
    out = critical_success(results, ModifierSpec(key="cs"), rng, faces=20)
...
def test_cs_with_compare_point():
    ...
    out = critical_success(
        results, ModifierSpec(key="cs", compare_point=">=18"), rng, faces=20
    )
    assert out[0].critical == "success"
...
def test_cf_default_marks_value_1():
    ...
    rng = SeededRNG(0)
    out = critical_failure(results, ModifierSpec(key="cf"), rng, faces=20)
...
def test_cf_with_compare_point():
    ...
    out = critical_failure(
        results, ModifierSpec(key="cf", compare_point="<=3"), rng, faces=20
    )
    assert out[0].critical == "failure"
...
def test_cs_and_cf_together():
    ...
    rng = SeededRNG(0)
    out = critical_success(results, ModifierSpec(key="cs"), rng, faces=20)
    out = critical_failure(out, ModifierSpec(key="cf"), rng, faces=20)
    assert out[0].critical == "success"
    assert out[1].critical == "failure"
    assert out[2].critical is None
...
def test_critical_empty_list():
    ...
    rng = SeededRNG(0)
    out = critical_success([], ModifierSpec(key="cs"), rng, faces=20)
    assert out == []
